...
public final class MusicSearch: NSCoding {
  ...
  // MARK: Properties
  public var results: [MusicElement]?
  ...
  public required init(json: JSON) {
    if let items = json[SerializationKeys.results].array { results = items.map { MusicElement(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    self.results = aDecoder.decodeObject(forKey: SerializationKeys.results) as? [MusicElement]
